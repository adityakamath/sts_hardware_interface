...
void SMS_STS::SyncWriteSpe(u8 ID[], u8 IDN, s16 Speed[], u8 ACC[])
{
	...
	for (u8 i = 0; i < IDN; i++) {
		...
		bBuf[1] = 0;
		bBuf[2] = 0; // SMS_STS_GOAL_POSITION (regs 42-43, unused in wheel mode)
		bBuf[3] = 0;
		bBuf[4] = 0;                                // SMS_STS_GOAL_TIME (regs 44-45)
...
{
	if (ID == -1) {
		return ServoUtils::readSignedWordFromBuffer(Mem, SMS_STS_PRESENT_POSITION_L - SMS_STS_PRESENT_POSITION_L,
...
{
	if (ID == -1) {
		return ServoUtils::readSignedWordFromBuffer(Mem, SMS_STS_PRESENT_SPEED_L - SMS_STS_PRESENT_POSITION_L,
...
{
	if (ID == -1) {
		return ServoUtils::readSignedWordFromBuffer(Mem, SMS_STS_PRESENT_LOAD_L - SMS_STS_PRESENT_POSITION_L,
...
{
	if (ID == -1) {
		return ServoUtils::readSignedWordFromBuffer(Mem, SMS_STS_PRESENT_CURRENT_L - SMS_STS_PRESENT_POSITION_L,
