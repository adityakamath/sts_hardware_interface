 ...
 *
 * This class eliminates several instances of manual buffer management:
 * - SMS_STS::SyncWritePosEx, SyncWriteSpe, SyncWritePwm
 *
 * Benefits:
 * - Automatic cleanup (no memory leaks)
...
class SyncWriteBuffer
{
  ...
  public:
	...
	SyncWriteBuffer(size_t motorCount, size_t payloadSize)
	    : buffer(nullptr), capacity(motorCount * payloadSize), bytesPerMotor(payloadSize), numMotors(motorCount)
	{
		if (capacity > 0) {
	 ...
	 */
	const u8* getBuffer() const { return buffer; }
	 ...
	 */
	size_t getBytesPerMotor() const { return bytesPerMotor; }

	/**
	 * @brief Get number of motors
	 */
	size_t getNumMotors() const { return numMotors; }
